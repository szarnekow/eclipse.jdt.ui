...
public class WorkingSetDropAdapter extends JdtViewerDropAdapter implements TransferDropTargetListener {
	...
	private void performWorkingSetReordering() {
		...
		if (index != -1) {
			if (fLocation == LOCATION_AFTER)
				index++;
			List<Object> result= new ArrayList<Object>(activeWorkingSets.size());
			List<Object> selected= new ArrayList<Object>(Arrays.asList(fElementsToAdds));
	...
	private void performElementRearrange(int eventDetail) {
		...
		if (eventDetail == DND.DROP_MOVE) {
			MultiElementSelection ms= (MultiElementSelection)fSelection;
			Map<IWorkingSet, List> workingSets= groupByWorkingSets(ms.getAllTreePaths());
			for (Iterator<IWorkingSet> iter= workingSets.keySet().iterator(); iter.hasNext();) {
				IWorkingSet ws= iter.next();
				List toRemove= workingSets.get(ws);
	...
	private Map/*<List<IWorkingSet>>*/<IWorkingSet, List> groupByWorkingSets(TreePath[] paths) {
		Map<IWorkingSet, List> result= new HashMap<IWorkingSet, List>();
		for (int i= 0; i < paths.length; i++) {
			TreePath path= paths[i];
			IWorkingSet ws= (IWorkingSet)path.getSegment(0);
